...
**0.0.5d** Rust-backed acceleration update. May 1st, 2026
- Added native Rust kernels and opt-in batched model-fitting fast paths for supported `commonfactorGWAS()` and `userGWAS()` DWLS workflows.
- Kept the existing R output shape and existing lavaan fallback behavior for unsupported inputs. Set `options(GenomicSEM.fast_strict=TRUE)` to turn unsupported native-path inputs into errors during validation, or use `options(GenomicSEM.fast_diagnostics=TRUE)` / the `GenomicSEM.fast_path` result attribute to verify which path ran.
- Added native compilation and initialization for supported `userGWAS()` fast-path models, removing the one-time lavaan setup pass before the Rust batch solver. Set `options(GenomicSEM.fast_usergwas_native_setup=FALSE)` to force the prior lavaan-assisted setup for A/B checks or debugging.
- Accelerated raw summary-stat preparation:
  - `munge()` and `sumstats()` now stream serial inputs one file at a time, use `data.table::fread()` by default via `options(GenomicSEM.fast_table_read=TRUE)`, and run supported row-QC / allele / Z / beta-SE transforms in Rust via `options(GenomicSEM.fast_munge_qc=TRUE)` and `options(GenomicSEM.fast_sumstats_qc=TRUE)`.
  - Supported multi-file `munge()` calls now use one batched Rust prep engine when possible, sharing the reference index across files and using Rust worker threads for batched compressed output. Tune the native batch width with `options(GenomicSEM.fast_munge_threads=...)`.
  - Supported multi-file `sumstats()` calls now use one batched Rust prep engine when possible, sharing the reference index and performing the cross-trait listwise SNP merge natively before returning the final multivariate table. Tune the native batch width with `options(GenomicSEM.fast_sumstats_threads=...)`.
  - `ldsc()` and `s_ldsc()` now use faster `fread()`-based chromosome, weight, M, and trait-table ingestion via `options(GenomicSEM.fast_ldsc_read=TRUE)`, plus a Rust block-crossproduct kernel for jackknife setup via `options(GenomicSEM.fast_ldsc_blocks=TRUE)` with up to 4 Rust worker threads by default.
- Cleaned package metadata and repaired stale help-page signatures for the public API so `R CMD check` surfaces fewer legacy packaging issues around the new release.
- Replaced broad package-wide imports with explicit namespace imports and cleaned residual `R CMD check` diagnostics, including several latent bugs in older helper paths that the diagnostics exposed.
