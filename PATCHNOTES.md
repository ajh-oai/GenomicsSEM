...
The same development line also accelerates raw summary-stat preparation. `munge()` and `sumstats()` now stream serial inputs one file at a time, use a faster `data.table::fread()` parser by default via `options(GenomicSEM.fast_table_read=TRUE)`, and run supported row-QC/allele/Z/beta-SE transforms in Rust via `options(GenomicSEM.fast_munge_qc=TRUE)` and `options(GenomicSEM.fast_sumstats_qc=TRUE)`. `ldsc()` and `s_ldsc()` also use faster `fread()`-based chromosome, weight, M, and trait table ingestion by default via `options(GenomicSEM.fast_ldsc_read=TRUE)`.
