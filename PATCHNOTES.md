...
Supported `userGWAS()` fast-path models are now compiled and initialized natively, so those runs no longer need the one-time lavaan setup pass before entering the Rust batch solver. Set `options(GenomicSEM.fast_usergwas_native_setup=FALSE)` to force the previous lavaan-assisted setup for A/B checks or debugging.

The same development line also accelerates raw summary-stat preparation. `munge()` and `sumstats()` now stream serial inputs one file at a time, use a faster `data.table::fread()` parser by default via `options(GenomicSEM.fast_table_read=TRUE)`, and run supported row-QC/allele/Z/beta-SE transforms in Rust via `options(GenomicSEM.fast_munge_qc=TRUE)` and `options(GenomicSEM.fast_sumstats_qc=TRUE)`. `ldsc()` and `s_ldsc()` also use faster `fread()`-based chromosome, weight, M, and trait table ingestion by default via `options(GenomicSEM.fast_ldsc_read=TRUE)`, plus a Rust block crossproduct kernel for jackknife setup via `options(GenomicSEM.fast_ldsc_blocks=TRUE)` and up to 4 Rust worker threads by default.
