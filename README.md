...
**Rust-backed fast paths**: Source installs now build native Rust kernels with Cargo. Low-level kernels, native row-QC for supported `munge()`/`sumstats()` prep paths, and faster table reading for `munge()`, `sumstats()`, `ldsc()`, and `s_ldsc()` are enabled by default when available. Experimental batched model-fitting fast paths for supported `commonfactorGWAS()` and `userGWAS()` DWLS runs can be enabled with `options(GenomicSEM.fast_commonfactor_fit=TRUE)` and `options(GenomicSEM.fast_usergwas_fit=TRUE)`. Use `options(GenomicSEM.fast_diagnostics=TRUE)` to confirm which path was used; returned objects also include `GenomicSEM.fast_path` attributes.
